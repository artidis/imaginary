...
func (s *AzureImageSource) DownloadImage(container, key, zone string) ([]byte, error) {
	session, err := newAzureSession(container)
	if err != nil {
		return nil, fmt.Errorf("azure: error getting azure session: %w", err)
	}
...
func (s *AzureImageSource) UploadImage(data []byte, fileKey, container, zone string) error {
	session, err := newAzureSession(container)
	if err != nil {
